...
% =============================== 8. INTEGRATION ==========================
\section{Integrating a Loop into a Task}
\label{sec:integrate}
Loop engineering is applied by answering six questions about a task---one per
contract---and binding each answer to an implementation. We give the recipe,
then a worked example and the surfaces it plugs into.

\subsection{Recipe: from a task to a loop}
\begin{enumerate}[leftmargin=1.5em,itemsep=2pt,topsep=2pt,parsep=0pt]
\item \textbf{State the goal $G$ as a check, not a wish.} Name the command or
predicate true exactly when the task is done (``\texttt{pytest} exits $0$'', ``no
\texttt{TODO} markers remain''). If you cannot write the check, the task is not
yet loop-ready.
\item \textbf{Define the work source $W$.} Decide how an iteration discovers the
next unit (parse failing tests, scan open items, pull a queue), skipping
anything already in the done-set.
\item \textbf{Implement the actor $A$.} The smallest step that advances one
unit---usually a single model call with a focused prompt, or a deterministic
edit. Keep it small so a bad step is cheap to discard.
\item \textbf{Wire the sensor(s) $S$ \emph{before} the actor.} At least one
verifier with evidence; prefer a fast computational sensor (tests, linter), add
an inferential one (a review agent) only where semantics matter.
\item \textbf{Choose the state store $M_s$.} The default JSON store suffices;
pick a \emph{stable} done-key per work item so resumption never redoes work.
\item \textbf{Set stop conditions $T$.} Always \texttt{GoalMet} plus a real bound
(\texttt{MaxIterations}, a token budget, or \texttt{LoopUntilDry}); never ship a
loop without one.
\end{enumerate}
Steps 1, 4, and 6 are the non-negotiable ones: they establish the preconditions
of P1 and P2. Isolation and budget are optional policy layered on top.

\subsection{Worked example: drive a test suite to green}
...
isolation: worktree
\end{lstlisting}
A runtime loads the spec and executes the rule of Fig.~\ref{fig:rule}: each pass
finds one failing test, the actor proposes a fix, \emph{both} sensors must pass
for it to count as done, and the run halts at \texttt{goal\_met}, the iteration
cap, or the token budget. Because state is persisted every pass, an interrupted
run resumes where it stopped (\texttt{onloop resume}).

\subsection{Where a loop plugs in}
The same loop integrates into different surfaces by changing only \emph{when} it
runs and \emph{what bounds} it: a \emph{scheduled} job (run nightly until
\texttt{dry}); a \emph{pre-merge gate} (run once; fail the build unless
\texttt{goal\_met}); a \emph{batch migration} (one work item per file, with
\texttt{isolation: worktree} so parallel actors do not collide); or a
\emph{supervised} run (a human-on-the-loop checkpoint every $k$ iterations). The
contract bindings are unchanged; only $T$ and the trigger differ.

\subsection{Pitfalls}
The recurring failures each violate a precondition of P1 or P2: an unverifiable
goal (no check); the actor running before any sensor exists (fast, unchecked
mistakes); unstable done-keys (work repeats on resume); and a missing real bound
(an effectively infinite loop). The schema-level obligations of
Sec.~\ref{sec:onloop} catch the last two at load time.
