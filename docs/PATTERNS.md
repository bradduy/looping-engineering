...
A catalog of loop shapes that `loopeng`'s contracts are meant to express. The
...
| File | Role | Changes per iteration? | `loopeng` mapping |
|---|---|---|---|
...
its own work before committing. That is exactly the failure mode `loopeng`
...
> Ralph is the loop with the brakes optional. `loopeng` is the same loop with
> the brakes required.
