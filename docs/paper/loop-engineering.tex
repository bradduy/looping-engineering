...
\[
\texttt{step}:\; M \times \mathit{Ctx} \;\to\; (a,\, v),
\]
its \textbf{input} is the model $M$ together with the \emph{context}
$\mathit{Ctx}$ the harness assembles for this step---the relevant guides
(feedforward), the available tools, retrieved memory, and the current task. Its
\textbf{output} is a single action $a$ (an edit or tool call) paired with the
sensor verdict $v$ on it (feedback). Harness engineering is precisely the design
of what enters $\mathit{Ctx}$ and which sensors produce $v$; it transforms raw
model capability into a reliable, \emph{checked} step. The relationship to the
loop is exact: the loop's per-run signature
($\texttt{run}$, Sec.~\ref{sec:loop}, Fig.~\ref{fig:io}) is the closure of
$\texttt{step}$ iterated under the control flow of $T$. Where harness I/O asks
``is this one step good?'', loop I/O asks ``is the whole goal met, and when do
we stop?''.
