...
\small
\begin{gathered}
\texttt{run}:\;\underbrace{(G,W,A,S,M_s,T)}_{\text{config}}\\[2pt]
\times\;\underbrace{(\mathsf{Wksp},\, M,\, \mathsf{Budget})}_{\text{environment}}
\;\to\;
...
\subsection{Inputs and outputs}
\label{sec:harness-io}
The harness has an I/O contract too, but a per-\emph{step} one. Treating a single
agent step as a function,
\[
\texttt{step}:\; M \times \mathit{Ctx} \;\to\; (a,\, v,\, \Delta\mu,\, \mathrm{tr}),
\]
where the harness both \emph{assembles} the input context $\mathit{Ctx}$
(feedforward) and \emph{checks} the resulting action (feedback);
Fig.~\ref{fig:hio} expands it.

\noindent\textbf{Input: the model $M$ and the context $\mathit{Ctx}$.} The model
supplies capability; $\mathit{Ctx}$ is everything the harness packs into the
window for this step:
\begin{itemize}[leftmargin=1.2em,itemsep=1pt,topsep=2pt,parsep=0pt]
\item \emph{Task} --- the current instruction or subtask.
\item \emph{Guides} (feedforward) --- conventions, architectural rules,
templates, linter configuration: constraints applied \emph{before} generation.
\item \emph{Tools} --- the action interface available this step (edits, shell,
search, MCP servers).
\item \emph{Memory} --- retrieved relevant state: prior decisions, accumulated
learnings (\texttt{AGENTS.md}), code excerpts.
\item \emph{Sandbox} --- the execution environment and its permissions.
\end{itemize}
Assembling this within the context window is itself the engineering work
(``context engineering'').

\noindent\textbf{Output: a checked step $(a, v, \Delta\mu, \mathrm{tr})$.}
\begin{itemize}[leftmargin=1.2em,itemsep=1pt,topsep=2pt,parsep=0pt]
\item $a$ --- a single action (edit, tool call, message), possibly blocked or
rewritten by a guardrail before it takes effect.
\item $v$ --- the sensor verdict on $a$, \emph{with evidence}: a conjunction of
\emph{computational} sensors (deterministic, fast: linters, tests,
type-checkers) and \emph{inferential} sensors (semantic, slower: review agents,
LLM judges).
\item $\Delta\mu$ --- side effects: the mutated workspace if $a$ is applied, and
any learnings written back to memory.
\item $\mathrm{tr}$ --- telemetry for observability (the step's cost, decision,
and verdict).
\end{itemize}

Harness engineering is precisely the design of \emph{what enters $\mathit{Ctx}$}
(feedforward) and \emph{which sensors produce $v$} (feedback); it turns raw model
capability into a reliable, \emph{checked} step. The relationship to the loop is
exact: the loop's per-run signature ($\texttt{run}$, Sec.~\ref{sec:loop},
Fig.~\ref{fig:io}) is the closure of $\texttt{step}$ iterated under the control
flow of $T$, threading each $\Delta\mu$ into the next step's $\mathit{Ctx}$ and
aggregating each $v$ into verified progress. Where harness I/O asks ``is this one
step good?'', loop I/O asks ``is the whole goal met, and when do we stop?''.

\begin{figure}[t]
\centering
\begin{tikzpicture}[font=\footnotesize, >=Latex, node distance=5mm,
  io/.style={draw, rounded corners, align=left, text width=0.86\columnwidth, inner sep=4pt},
  stepn/.style={draw, very thick, draw=blue!55!black, rounded corners, fill=blue!8, align=center, text width=0.86\columnwidth, inner sep=4pt}]
\node[io, fill=blue!5] (in) {\textbf{Input: model $+$ assembled context $\mathit{Ctx}$}\\[1pt]
model $M$ $\cdot$ task $\cdot$ guides (feedforward) $\cdot$ tools $\cdot$
memory $\cdot$ sandbox};
\node[stepn, below=of in] (step) {\textbf{Harness} \texttt{step}\\[1pt]
assemble $\mathit{Ctx}$ $\to$ invoke $M$ $\to$ run sensors
(computational $+$ inferential) $\to$ guard};
\node[io, fill=orange!8, below=of step] (out) {\textbf{Output: one checked step}\\[1pt]
action $a$ (gated) $\cdot$ verdict $v$ $+$ evidence $\cdot$ memory update
$\Delta\mu$ $\cdot$ telemetry $\mathrm{tr}$};
\draw[->,thick] (in) -- (step);
\draw[->,thick] (step) -- (out);
\end{tikzpicture}
\caption{Inputs and outputs of a harness \emph{step} (cf.\ the loop's per-run
I/O, Fig.~\ref{fig:io}). Harness engineering designs the feedforward (what fills
$\mathit{Ctx}$) and the feedback (which sensors yield $v$), turning model
capability into one reliable, checked action. A loop iterates this step until
$T$ fires.}
\label{fig:hio}
\end{figure}
...
% =============================== 8. INTEGRATION ==========================
\section{Putting a Loop to Work (Practitioner's Guide)}
\label{sec:integrate}
You do not need the formalism above to use a loop. In practice you answer six
plain questions about your task, write the answers in a short spec file, and let
the runtime drive the agent. This section is hands-on: a recipe in everyday
terms, an example you can run today, a table to map your own task, and the
mistakes to avoid.

\subsection{The six questions}
Each maps to one part of a loop (named in parentheses); you can ignore the
symbols and just answer the question.
\begin{enumerate}[leftmargin=1.5em,itemsep=2.5pt,topsep=2pt,parsep=0pt]
\item \textbf{When is it done?} (goal) Give a command that succeeds or a check
that comes back clean---``\texttt{pytest} passes,'' ``the linter reports zero
issues.'' If you cannot state the check, the task is not ready for a loop yet.
\item \textbf{What are the pieces of work?} (work source) Say how to list what
still needs doing---e.g.\ the currently failing tests, or the files not yet
migrated. Finished pieces are skipped automatically.
\item \textbf{How do you do one piece?} (actor) Usually: ask the model to fix
\emph{one} thing with a focused prompt. Handle a single piece per step so a bad
attempt is cheap to throw away.
\item \textbf{How do you check a piece really worked?} (sensor) A test or linter
that returns a clear pass/fail with output. \emph{Set this up before the actor}:
without a check, the loop just makes mistakes faster. You may use several.
\item \textbf{How is progress remembered?} (state) Use the built-in file store;
just give each piece a stable name so a restart picks up where it left off
instead of redoing work.
\item \textbf{When should it stop?} (stop rule) When the goal is met, after $N$
attempts, or at a token budget---whichever comes first. Always set at least one;
a loop with no stop rule runs forever.
\end{enumerate}
Questions 1, 4, and 6 are the ones you must not skip: a checkable goal, a
verifier wired before the actor, and a real stop limit.

\subsection{Run one in two minutes}
A self-contained example ships with the framework and needs no API key---it
marks a list of items done, checking each on disk. Try it:
\begin{lstlisting}[language=bash]
pip install -e '.[dev]'
onloop run examples/punchlist/punchlist.loop.yaml \
           --workspace /tmp/demo
# stopped: goal_met   iterations: 3
# done: ['inspect', 'paint', 'wire']
onloop resume ...   # continues from saved progress
\end{lstlisting}
For a real coding task you swap in your own pieces. The same spec for ``drive a
failing test suite to green'' looks like this---each line names the code that
answers one of the six questions:
\begin{lstlisting}
name: make-tests-green
goal:        { uses: myloops.AllTestsPass,
               with: { cmd: "pytest -q" } }   # done?
work_source: { uses: myloops.FailingTests }   # pieces
actor:       { uses: myloops.FixOneTest,
               with: { model: <llm> } }       # do one
sensors:
  - { uses: myloops.PytestSensor }            # check
stop:
  - { uses: onloop.stops.GoalMet }
  - { uses: onloop.stops.MaxIterations,
      with: { max_iters: 25 } }               # limit
budget: { max_tokens: 400000 }
\end{lstlisting}
\texttt{onloop run} then repeats: find one failing test, let the model fix it,
re-run the test, and record it---stopping at green, 25 tries, or the budget. If
it is interrupted, \texttt{onloop resume} continues.

\subsection{Map your own task}
Most jobs fit the same shape; you only change three answers (Table~\ref{tab:map}).

\subsection{Where it runs}
The \emph{same} loop fits different places by changing only when it runs and its
limit: a nightly job that runs until nothing new is found; a pull-request check
that runs once and blocks the merge unless the goal is met; a one-off bulk
migration (one file per piece, run in isolated copies so parallel fixes do not
clash); or a supervised run that pauses for a human to approve every few steps.

\subsection{Common mistakes}
Four mistakes account for most trouble: (i) no way to check ``done'' (the loop
cannot tell when to stop or whether a fix worked); (ii) letting the model act
before a check exists (fast, unverified changes); (iii) not giving each piece a
stable name (it gets redone after a restart); and (iv) forgetting a stop limit
(it never ends). The framework refuses to run a spec that has no check or no
stop limit, so mistakes (i), partly, and (iv) are caught before the loop starts.
...
\begin{table*}[t]
\centering
\caption{Mapping everyday tasks onto a loop. You change only three answers---what
``done'' means, what one piece of work is, and how to check it; the rest of the
machinery (do one piece, remember, stop) stays the same.}
\label{tab:map}
\small
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{@{}>{\bfseries}p{0.20\textwidth} p{0.24\textwidth} p{0.22\textwidth} p{0.26\textwidth}@{}}
\toprule
...
Close all TODOs     & no \texttt{TODO} comments remain & one \texttt{TODO} comment  & the TODO is gone and tests still pass \\
